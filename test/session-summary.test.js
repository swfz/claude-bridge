...
describe("summarizeHead", () => {
  ...
  it("strips injected tags and collapses whitespace", () => {
    ...
    assert.equal(
      summarizeHead(lines).firstUserMessage,
      "Another Claude session sent a message: レビューお願い"
    );
  });

  it("picks up the cwd written in the JSONL", () => {
    const lines = jsonl([
      userRecord("依頼", { cwd: "/home/me/gh/claude-bridge" }),
    ]).split("\n");
...
describe("readSessionSummaryFor", () => {
  it("resolves the JSONL path from cwd and sessionId", async () => {
    ...
    await writeFile(
      join(projects, "-home-me-work", "sid-1.jsonl"),
      jsonl([{ type: "ai-title", aiTitle: "作業中" }])
    );
