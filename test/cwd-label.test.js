import { describe, it } from "node:test";
import assert from "node:assert/strict";
import { parseCwd } from "../client/src/utils/cwdLabel.js";

describe("parseCwd", () => {
  it("handles missing input", () => {
    assert.deepEqual(parseCwd(null), { project: "", worktree: null });
    assert.deepEqual(parseCwd(undefined), { project: "", worktree: null });
    assert.deepEqual(parseCwd(""), { project: "", worktree: null });
    assert.deepEqual(parseCwd(42), { project: "", worktree: null });
  });

  it("extracts project and worktree from a .claude/worktrees path", () => {
    assert.deepEqual(
      parseCwd("/home/sawafuji/gh/claude-bridge/.claude/worktrees/optimized-cooking-garden"),
      { project: "claude-bridge", worktree: "optimized-cooking-garden" }
    );
  });

  it("ignores subdirectories inside a worktree", () => {
    assert.deepEqual(
      parseCwd("/home/u/gh/claude-bridge/.claude/worktrees/foo/server"),
      { project: "claude-bridge", worktree: "foo" }
    );
  });

  it("recognizes a bare .worktrees marker segment", () => {
    assert.deepEqual(parseCwd("/home/u/proj/.worktrees/x"), {
      project: "proj",
      worktree: "x",
    });
  });

  it("recognizes a bare worktrees marker segment", () => {
    assert.deepEqual(parseCwd("/home/u/repo/worktrees/bar"), {
      project: "repo",
      worktree: "bar",
    });
  });

  it("falls back to the last segment for a normal path", () => {
    assert.deepEqual(parseCwd("/home/u/gh/claude-bridge"), {
      project: "claude-bridge",
      worktree: null,
    });
  });

  it("ignores trailing slashes and empty segments", () => {
    assert.deepEqual(parseCwd("/a/b/"), { project: "b", worktree: null });
    assert.deepEqual(parseCwd("/a//b//"), { project: "b", worktree: null });
  });

  it("falls back to the last segment when the marker has no preceding project segment", () => {
    assert.deepEqual(parseCwd("/worktrees/bar"), {
      project: "bar",
      worktree: null,
    });
  });
});
