...
const dropBridge = (arr, marker) => (arr || []).filter((h) => !JSON.stringify(h).includes(marker));
...
if (existsSync(sessionStart)) {
  settings.hooks.SessionStart = dropBridge(settings.hooks.SessionStart, 'bridge-session-start');
