...
export default function ChoicePrompt({
  ...
  return (
    <div className={`choice-prompt choice-prompt-${prompt.kind}`}>
      <div className="choice-prompt-head">
        <span className="choice-prompt-badge">
          {kindLabel(prompt, waitingFor)}
        </span>
        {prompt.multiSelect && (
          <span className="choice-prompt-note">複数選択できます</span>
        )}
        {prompt.tabs?.items?.length > 1 && (
          <span className="choice-prompt-note">
            {prompt.tabs.items.map((t) => t.label).join(" / ")}
          </span>
        )}
      </div>

      {prompt.question && (
        <div className="choice-prompt-question">{prompt.question}</div>
      )}
