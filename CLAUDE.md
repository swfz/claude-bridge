...
- `jsonl-utils.js` -- 共通ユーティリティ。`extractTextContent`, `extractToolUses`, `CLAUDE_PROJECTS_DIR` 等。jsonl-watcher.js と claude-sessions.js から参照。**`extractContextUsage(record)` はコンテキスト使用量**（詳細は「コンテキスト使用量の表示」の節）
...
- `activity-heatmap.js` -- ホーム画面の活動ヒートマップ（草）。`~/.claude/projects/**/*.jsonl` を全部読み、**ローカル日付**で「ユーザーの実プロンプト数 / assistant の応答数 / トークン（input・output・cache_creation・cache_read）」を日別に足す。サブエージェントの `subagents/agent-*.jsonl` も対象（本体側には書かれないので二重計上にはならない）。tool_result の user レコードは人が打ったものではないので数えない。全走査は 900MB 弱／9 秒かかるので、**ファイル単位の日別集計を `<dataDir>/activity-heatmap.json` に永続化**し、2 回目以降は追記分（前回オフセット以降）だけ読み足す（末尾の書きかけ行は最後の改行までで打ち切る／ファイルが縮んでいたら先頭から読み直す）。同じ走査が並行しないよう in-flight の Promise を共有する。**`listActiveSessionIds({from, to})`** は同じ集計を使って「その期間（ローカル日付）に活動があったセッション ID の集合」を返す＝ホームの一覧を活動グラフの棒で絞り込むときのデータ源（JSONL のパスの `parts[1]` から ID を取り、`<sessionId>/subagents/agent-*.jsonl` の活動は親セッションに帰属させる）。mtime を使わないのは「最後に更新した日」でしかなく、その日に活動して後日また続けたセッションが漏れるため
...
- `ContextMeter.jsx` -- ヘッダーの `RateLimitMeter` の左隣に出す「今このセッションが使っているコンテキスト量」のメーター（`ctx 62%`）。`usage` を描くだけの純表示コンポーネントで、null なら非表示。バー色のしきい値は `RateLimitMeter` と同じ（`utils/contextUsage.js` の `contextColorFor`）、ツールチップにトークン数と input / cache 作成 / cache 読み込みの内訳とモデル。作業中タブを見ているときだけ出す（ホーム表示中は出さない）
- `ActivityPanel.jsx` -- ホーム最上段の活動パネル。`activity_heatmap`（365 日分）を描くだけの純表示コンポーネントで、集計・整形は `utils/heatmap.js` の純粋関数が持つ。**同じデータを 5 ビューで見せる**（`HeatmapView` / `BarsView`（日別・週別・月別を粒度で切り替え） / `WeekdayView` の小コンポーネントを 1 ファイルに同居）。ビュー・メトリック（メッセージ / トークン）・開閉は localStorage（`homeActivityView` / `homeHeatmapMetric` / `homeHeatmapOpen`）に覚える。**棒・升目はクリックでその期間を選ぶボタン**で、選択は `selectedPeriod` / `onSelectPeriod`（`{from, to, label} | null`）で親と受け渡しする（同じものを再クリックで解除。凡例の右に `絞り込み中: <label> ×` を出す）。パネル自身は期間を持たず、一覧の絞り込みは `HomeView` → `App` 側の仕事
  - **草**: 1 列 = 1 週（GitHub と同じ日曜始まり）。**濃淡のしきい値は固定値ではなく「活動があった日」の分位（p25/p50/p75）**。日によって桁が違う（トークンは特に）ため。色は `--heat-0`〜`--heat-4`（ライト/ダークで別値）
  - **日別 / 週別 / 月別**: 1 本 = 1 日 / 1 週（**月曜始まり**。草だけが GitHub に合わせた日曜始まり） / 1 月の縦棒。畳み方は `bucketDays(days, 'day'|'week'|'month')` で、`from`/`to` は実際に含まれる日なので先頭・末尾は欠けた週・月になる。棒の幅は本数に合わせて `barWidthFor()` が決める（3px / 10px / 28px）。濃淡では潰れる量の差を高さで読むビューなので、**分位ではなく最大値に対する線形**にする（草と役割を分ける）。棒と月の目盛りは同じ `grid-template-columns: repeat(var(--count), var(--bar))` を共有するので必ず日と揃う
  - **曜日**: 月曜始まりの横棒 7 本（草だけが日曜始まりなのは GitHub に合わせているため）。合計値と「その曜日で活動があった日数」を添える
  - 草と棒（日別・週別・月別）は横に長いのでこのブロックだけ横スクロールし、初期位置は右端（最新）。**左端の曜日ラベル／y 軸目盛りは `position: sticky; left: 0` で貼り付ける**（スクロールすると流れて消えるため。背景色を敷かないと升目が透ける）
...
「今そのセッションがコンテキスト窓をどれだけ使っているか」をヘッダー・サイドバーのタブ・ホームの一覧に出す。

- **データ源は JSONL の `type: "assistant"` レコードの `message.usage`**。**直近の assistant の `input_tokens + cache_creation_input_tokens + cache_read_input_tokens` が今の文脈量**（Claude Code の statusline と同じ考え方。`output_tokens` は次ターンの入力になるまで含まれない）。API エラー等で `usage` を持たないレコードは飛ばし、末尾から遡って usage のある直近を使う
- **コンテキスト窓の上限は `message.model` からのヒューリスティック**（JSONL には窓サイズが書かれない）。`[1m]` 付き、および opus 4.6 以降・opus 5 以降・fable / mythos 系は 1,000,000、sonnet / haiku / 不明は 200,000。判定は `contextWindowFor(model, contextTokens)` で、**実測の文脈量が窓を超えていれば 1M に繰り上げる**（モデル表が古くなっても 100% に張り付かない）。表を 1M 側に寄せてあるのは、この環境の JSONL で `[1m]` 表記なしに 700k〜900k の文脈量が実測されたため（200k 既定だと大半が 100% になり意味を失う）。正確な窓サイズは statusline の `context_window.context_window_size` にしか無いので、必要になれば tee で横流しする拡張余地がある
- usage が全部 0 のレコード（API を呼んでいない `<synthetic>` のエラー表示等）は文脈量として無効なので null 扱いで飛ばす
- **`/compact` 直後は次の応答まで古い（大きい）値が残る**。usage は assistant レコードにしか書かれないので、圧縮された文脈の実サイズは次の応答が来て初めて分かる
- 抽出は `server/jsonl-utils.js` の `extractContextUsage(record)`（`{inputTokens, cacheCreationTokens, cacheReadTokens, outputTokens, contextTokens, contextWindow, model}` か null）。会話列には `parseHistoryLines()` と `JsonlWatcher` が assistant メッセージの `contextUsage` として添え、ホームの一覧には `summarizeTail()` が summary の `contextUsage` として乗せる
- 表示側の整形は `client/src/utils/contextUsage.js` の純粋関数（`latestContextUsage` / `contextPercent` / `formatTokens` / `contextColorFor`）。ヘッダーは `ContextMeter.jsx`、タブは `.tab-context`、ホームはカードのメタ行と行リストの `Ctx` 列
...
- **期間絞り込み（活動グラフの棒・升目のクリック）**: `list_recent_sessions {days, starred, period: {from, to}}`。`period` があるときは mtime ではなく `listActiveSessionIds()`（ヒートマップのファイル別日別集計）で絞る＝「その棒に数えられたセッション」と一覧を一致させるため（mtime は最後の更新日でしかなく、その日に動いて後日続けたセッションが漏れる）。サーバーは `sanitizePeriod()` で `YYYY-MM-DD` かつ `from <= to` を検証し、不正なら従来どおり日数で返す。応答は `recent_sessions {days, period?, sessions}`。**選んだ期間は localStorage に保存しない**（その場限りの絞り込み）。日数プリセットを押すと解除され、絞り込み中はプリセットを光らせず隣に `.home-period-chip`（`<label> ×`）を出す。応答は `period` をエコーするので、クライアントは今の選択と食い違う応答を捨てる（期間付きは走査待ちで遅く、解除後の速い応答に追い越され得る）。升目・棒の button は `tabIndex=-1`（数百個が Tab の停止点になるのを避ける）
...
- **コンテキスト使用量**も上下段に出す。上段はカードのメタ行に `ctx 62%`、下段は「Artifact」と「更新」の間の幅 44px の `Ctx` 列（色は `contextColorFor`、tooltip に内訳）。データは summary の `contextUsage`（`server/session-summary.js` の `summarizeTail`）
...
- **最上段の活動パネル**: `list_activity_heatmap {days}` → `activity_heatmap {days, total, generatedAt}`（`server/activity-heatmap.js`）。直近 365 日を草 / 日別・週別・月別の棒 / 曜日別の 5 ビューで見せ、メッセージ数とトークン数を切り替えられる（ビューを変えてもサーバーには取りに行かない。5 つとも同じ 365 日分から計算する）。棒・升目のクリックは下段の期間絞り込みになる。取得はホームを開いた時と「更新」時だけ（直近一覧と同じ理由）。**セッション一覧のどのセクションにも属さない全体の指標なので、見出しより前・画面の最上段に置く**。共有モードでもパスは出ないので隠していない
...
| `Alt+I`                                            | -（ドロワー表示中は効かない。`useGlobalKeys` が `.drawer-overlay` を見て後ろの送信欄へは飛ばさない）                                                                                                                                                                                                                                                                                                                                          | ピック中＝取消して送信欄へフォーカス／それ以外も送信欄へ                                   | 送信欄へフォーカス                                                                          |
